...
const subjects = ['DAA', 'Operating System', 'Computer Networks', 'Database Management'];
  ...
  return (
    <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      <div className="card" style={{ border: '1px solid #ddd', borderRadius: '8px', padding: '20px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '20px' }}>VIT Student Result Dashboard</h1>
        <form onSubmit={handleSubmit}>
          ...
          <button type="button" onClick={handleSearch} style={{ padding: '10px 20px', backgroundColor: '#4CAF50', color: 'white', border: 'none', borderRadius: '4px', marginBottom: '20px', cursor: 'pointer' }}>
            Search Student
          </button>

          {subjects.map((subject) => (
            <div key={subject} style={{ marginBottom: '20px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 'semibold', marginBottom: '10px' }}>{subject}</h2>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <input
                  placeholder="MSE Marks"
                  value={marks[subject].mse}
                  onChange={(e) => handleMarksChange(subject, 'mse', e.target.value)}
                  style={{ padding: '8px', borderRadius: '4px', border: '1px solid #ddd' }}
                />
                <input
                  placeholder="ESE Marks"
                  value={marks[subject].ese}
                  onChange={(e) => handleMarksChange(subject, 'ese', e.target.value)}
                  style={{ padding: '8px', borderRadius: '4px', border: '1px solid #ddd' }}
                />
              </div>
          ...
          <button type="submit" disabled={loading} style={{ width: '100%', padding: '10px 20px', backgroundColor: '#008CBA', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
